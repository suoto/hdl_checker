...
@contextmanager
def _createClientServerPair(params: Optional[InitializeParams]):
    ...
    server.diagnostics = []  # type: ignore

    @server.feature(features.TEXT_DOCUMENT_PUBLISH_DIAGNOSTICS)
    def serverPublishDiagnosticsHandler(diag):  # pylint: disable=unused-variable
        _logger.info("Publishing diagnostic: %s", diag)
        server.diagnostics.append(diag)
    ...
    def clientShowMessageHandler(*args):  # pylint: disable=unused-variable
        ...
        client.messages.append((type_, msg))
    ...
    client.lsp.send_request(features.INITIALIZE, params).result(LSP_REQUEST_TIMEOUT)

    _logger.info("Sending initialized request")
    client.lsp.send_request(features.INITIALIZED).result(LSP_REQUEST_TIMEOUT)
    ...
    shutdown_response = client.lsp.send_request(features.SHUTDOWN).result(
        LSP_REQUEST_TIMEOUT
    )
    ...
    #  assert False, "Timeout waiting for server thread to complete"
    #  client.shutdown()
...
class _LspHelper(unittest2.TestCase):
    ...
    def _createClientServerPair(self):
    ...
    def checkLintFileOnMethod(
        ...
        self.assertFalse(
            client.diagnostics, "Client shoult not have diagnostics by now"  # type: ignore
        )

        _logger.info("Patching %s with %s", "getMessagesByPath", list(expected_diags))
        with patch.object(
            server.checker, "getMessagesByPath", return_value=list(expected_diags),
        ):
            hdl_checker.utils.ENABLE_DEBOUNCE = False
            try:
                client.lsp.send_request(method, params).result(LSP_REQUEST_TIMEOUT)
            finally:
                hdl_checker.utils.ENABLE_DEBOUNCE = True

            self.assertTrue(client.diagnostics, "Expected client to have diagnostics")  # type: ignore
            diags: List[CheckerDiagnostic] = []
            while client.diagnostics:  # type: ignore
                diag = client.diagnostics.pop()  # type: ignore
    ...
    def test_LintFileOnOpen(self):  # pylint: disable=inconsistent-return-statements
        ...
        with self._createClientServerPair() as pair:  # pylint: disable=not-context-manager
            server, client = pair
            _logger.info("#" * 100)
            self._runDidOpenCheck(
                server, client, p.join(TEST_PROJECT, "another_library", "foo.vhd")
            )
...
class TestRootUriNoProjectFile(_LspHelper):
    ...
    def _createClientServerPair(self):
        return _createClientServerPair(
            InitializeParams(
                process_id=1234,
